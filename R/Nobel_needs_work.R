# Nobel_needs_work     pwr_comparison_1
library(gtools)
Sept 29
...
power_function<-function(n){
  ...
  for (i in 1:n) {
    ...
    #<-rmultinom(100, 100, prob= c(h[1]/sum(h), h[2]/sum(h),h[3]/sum(h),h[4]/sum(h)))
    cell_counts<-rmultinom(n, 1000, prob= c(MB[1,1], MB[1,2], MB[1,3], MB[1,4]))
    # generate joint distribution estimates
    ...
    population_gamma<-sqrt((p11hat+p12hat)*(p11hat+p21hat)*(p21hat+p22hat)*(p12hat+p22hat))
    Phi<-((p11hat*p22hat) - (p12hat*p21hat)) / population_gamma
